...
The PEG grammar supports `nodes`, `references`, `definitions`, `implementations`, `callers` (optionally `including dispatch`), and `callees` `of node where ...`, `search "prefix"`, and `unresolved calls`; see the [query guide](query.md). `--root` selects a module path, `--location` selects one registered checkout, and `--snapshot` selects one historical immutable snapshot. `--location` and `--snapshot` cannot be combined. `nodes` and `unresolved calls` without a location or snapshot use primary heads; the symbol operations read the head of every registered checkout in the selected roots.

The symbol operations resolve a selector to canonical symbols and read the postings of those symbols in each selected snapshot's active documents. One identity declared at several heads is one target whose declarations are listed separately; if a selector matches several identities in scope, the command returns candidate rows with their root, checkout, source position, and snapshot instead of choosing one arbitrarily. Select `--location`, `--snapshot`, or a narrower predicate to disambiguate. Every result lists the packages in scope that are `partial`, `syntax`, or `excluded`, because those hold no or only some proven facts. `unresolved calls` means a call locator is unresolved or names no declaration in the selected scope; it is not a compiler diagnostic.
