...
Discovery and hashing finish before publication; changed-file parsing and source-revision writes happen inside the publication transaction. All modules discovered in one `IndexModules` call publish in that transaction: a malformed Go file in a later nested module rolls back earlier root, location, revision, snapshot, and head writes from the call. Each snapshot is marked ready before its location head is advanced. Existing heads use a version-checked update; a concurrent writer changing the version makes publication fail instead of silently overwriting it. External `go.work` uses selected during the same `uir add` invocation are indexed in a separate `IndexModules` call and therefore a separate transaction; successful primary-path publication is not rolled back if that later call fails.
